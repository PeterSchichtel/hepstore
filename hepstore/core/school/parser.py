...
def hidden_layer_type(args):
    try:
        x, y = map(int, args.split(','))
        return (x, y)
    except:
        raise TypeError("--hidden_layers must be x,y")
    pass
...
class SchoolParser(ArgumentParser):
    ...
    def __init__(self,description="This school allows to learn from data in .npy format."):
        ...
        self.add_argument(      "--signal_labels",     default=["1.0"],     type=str,    help="specify labels used as signal in ROC curve", nargs='+')
        self.add_argument(      "--background_labels", default=["0.0"],     type=str,    help="specify labels used as background in ROC curve", nargs='+')
        ...
        self.add_argument(      "--tol",               default=0.0001, type=float)
        self.add_argument(      "--reg_param",         default=0.0, type=float)
        self.add_argument(      "--c_parameter",       default=1.0, type=float)
        self.add_argument(      "--kernel",            default='rbf', type=str)
        self.add_argument(      "--degree",            default=3, type=int)
        self.add_argument(      "--gamma",             default='auto')
        self.add_argument(      "--coef0",             default=0.0, type=float)
        self.add_argument(      "--shrinking",         action='store_false')
        self.add_argument(      "--probability",       action='store_false')
        self.add_argument(      "--cache_size",        default=200., type=float)
        self.add_argument(      "--verbose",           action='store_true')
        self.add_argument(      "--max_iter",          default=-1, type=int)
